...
export const getUserTasks = async (filterStatus = 'OPEN_TASKS', filters = {}) => {
    try {
        ...
        if (currentProfile.role === 'Administrador') {
            ...
            if (currentProfile.departamento_id) {
                // Tareas del departamento (públicas) + SIEMPRE tareas creadas por él + tareas asignadas a él
                ...
                // Sin departamento, solo sus propias tareas (creadas y asignadas)
            ...
            if (currentProfile.departamento_id) {
                ...
                if (responsableIds.length > 0) {
                    // Tareas del departamento excluyendo las creadas por responsables + SIEMPRE tareas creadas por él + tareas asignadas a él
                    ...
                    // No hay responsables, ver todas las tareas del departamento + SIEMPRE tareas creadas por él + tareas asignadas a él
                    query = query.or(`and(departamento.eq.${currentProfile.departamento_id},privada.eq.false),creador.eq.${currentProfile.id},asignado_a.eq.${currentProfile.id}`);
                }
            } else {
                // Sin departamento, solo sus propias tareas (creadas y asignadas)
    ...
    try {
        ...
        const { data: updatedTask, error: taskError } = await supabaseClient
            ...
            .update({
                ...
                departamento: newData.departamento === '' ? null : newData.departamento,
                asignado_a: newData.asignado_a === '' ? null : newData.asignado_a,
